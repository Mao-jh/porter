...
"$TS" -addr 127.0.0.1:54331 -name big.bin -size 1048576 -ftp > "$WORK/r1.log" 2>&1 &
R1=$!
sleep 1.5
...
"$P" "http://127.0.0.1:54331/file/big.bin" -o "$WORK/r1ref.bin" >/dev/null 2>&1
...
"$TS" -dir "$WORK" -addr 127.0.0.1:54332 -name big.bin -size 2097152 > "$WORK/r2.log" 2>&1 &
R2=$!
sleep 1.5
if "$P" "http://127.0.0.1:54332/hls/big.bin.enc.m3u8" -o "$WORK/hls_enc.bin" -state-dir "$WORK/st2" > "$WORK/r2dl.log" 2>&1; then ok "R2a: HLS AES-128 加密下载成功"; else bad "R2a: HLS 加密下载失败"; tail -3 "$WORK/r2dl.log"; fi
...
"$P" "http://127.0.0.1:54399/none.bin" -retry-forever -state-dir "$WORK/st3" -o "$WORK/r3.bin" > "$WORK/r3.log" 2>&1 &
R3PID=$!
sleep 12
...
"$TS" -dir "$WORK" -addr 127.0.0.1:54333 -name big.bin -size 8388608 -limit 51200 > "$WORK/r4slow.log" 2>&1 &
R4S=$!
"$TS" -dir "$WORK" -addr 127.0.0.1:54334 -name big.bin -size 8388608 > "$WORK/r4fast.log" 2>&1 &
R4F=$!
sleep 1.5
T0=$(date +%s)
if "$P" "http://127.0.0.1:54333/file/big.bin" -mirror "http://127.0.0.1:54334/file/big.bin" -min-rate 204800 -state-dir "$WORK/st4" -o "$WORK/r4.bin" > "$WORK/r4.log" 2>&1; then
  T1=$(date +%s)
...
"$P" "http://127.0.0.1:54334/file/big.bin" -o "$WORK/r4ref.bin" >/dev/null 2>&1

...
```go
// 分片计划
// 第 6 轮：显式分片数上限 16；本轮扩展档位至 MaxExplicitConnections = 128
//  （-n 参数，显式档位 16/32/64 常规收益区 / 128 极端弱网多拨），
...
- **MCP download\_probe**：第 5 个工具，`cli.ProbeURL` 语义同源（-proxy/-load-cookies/
  AllowRemote 透传），输出 size\_bytes/ranged/name。
...
- bencode 解码保留**原始字节区间**（`bnode.raw`）→ info\_hash = SHA1(raw) 逐字节正确

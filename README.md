...
### TUI
```bash
porter-tui                        # 交互界面：a 添加 / j·k 或 ↑↓ 选择 / p 暂停·继续 / d 删除 / i 详情 / o·O 打开 / q 退出（按键大小写均可）
...
- **鼠标交互**（Windows Terminal 等支持鼠标序列的终端）：点击任务行选中、点行尾 `[暂停]/[继续]/[删除]` 按钮直接操作、滚轮上下选择。
- **失败优先 + 全局汇总**：任务列表按「失败 → 暂停 → 进行中 → 完成」排序（同状态保持添加顺序），
  顶部汇总行实时显示活动任务数 / 总速度 / 总进度；点击失败行可展开完整错误详情（再点收起），行尾保留短标签。
- **完成即开**：选中已完成任务按 `o` 打开文件、`O` 打开所在目录（Windows 定位到文件）。
- **`i` 详情面板**：布局 A（窄窗）按 `i` 全高展开当前任务详情（吞吐面积图/分片图/元数据），
  `j`/`k` 可在详情内切换任务，`esc`/`i` 收起。
